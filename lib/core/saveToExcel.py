...
class saveToExcel:
    ...
    def save(self, lines,titles=[]):
        if len(titles)<=0:
            titles=[self.sheet.title]
    ...
    def setTitle(self, titles):
        for index,title in enumerate(titles):
            self.sheet.cell(self.Sheet_line, index+1).value = title
        self.Sheet_line += 1
        ...
        for line in lines:
            if isinstance(line,list):
                for index,value in enumerate(line):
                    self.sheet.cell(self.Sheet_line, index+1).value = str(value)
            else:
                self.sheet.cell(self.Sheet_line, 1).value = line
            self.Sheet_line += 1
